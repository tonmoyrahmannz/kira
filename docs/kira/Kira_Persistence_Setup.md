...
## What Hermes supports directly
Hermes auto-injects workspace bootstrap files into new runs. The key files are:
- `AGENTS.md`
- `SOUL.md`
- `TOOLS.md`
- `IDENTITY.md`
...
- `~/.hermes/config.yaml` (safe additive config updates):
...
- Hermes always injects the configured workspace, which starts with `AGENTS.md` reading the persistent context files
...
## Notes
- This approach is additive and does not break default Hermes behavior.
- If deeper customization is needed later, use Hermes hooks to mutate bootstrap context.
